...
```
helm install --name os-vim-driver <path to chart>
```

Configuration for the Helm deployment can be provided with a **Helm values file** on the `-f` option. 

```
helm install --name os-vim-driver <path to chart> -f <path to Helm values file>
```
...
Locally:
```
docker save -o os-vim-driver-img.tar os-vim-driver:<release version number>
```

Transfer the file using scp:
```
scp os-vim-driver-img.tar <user>@<worker-node-ip>:/home/<user>
```

On Node:
```
docker load -i /home/<user>/os-vim-driver-img.tar
```
...
```
docker:
  image: <registry host and port>/os-vim-driver
